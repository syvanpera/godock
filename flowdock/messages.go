...
// Stream the messages for the given flows.
//
// Flowdock API docs: https://flowdock.com/api/streaming and
// https://www.flowdock.com/api/messages
func (s *MessagesService) Stream(filters []string, token string) (chan Message, *eventsource.EventSource, error) {
	retryDuration := 3 * time.Second

	u := fmt.Sprintf("flows?filter=%v&access_token=%v", strings.Join(filters, ","), token)
